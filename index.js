...
app.post('/api/auth/login', async (req, res) => {
    try {
        ...
        if (user['2faEnable']) {
            ...
            return res.status(200).json({ tempToken, expiresInSecond: config.cacheTemporaryTokenExpiresInSeconds })
        } else {
...
app.post('/api/auth/refresh-token', async (req, res) => {
    try {
        ...
        const tokenRecord = await userRefreshTokens.findOne({ refreshToken, userId: decoded.userId })

        if (!tokenRecord) {
            return res.status(401).json({ message: 'Refresh token invalid or expired' })
        }

        await userRefreshTokens.remove({ _id: tokenRecord._id })
        await userRefreshTokens.compactDatafile()

        const newAccessToken = jwt.sign({ userId: decoded.userId }, config.accessTokenSecret, {
            subject: 'accessApi',
            expiresIn: config.accessTokenExpiresIn
        })

        const newRefreshToken = jwt.sign({ userId: decoded.userId }, config.refreshTokenSecret, {
            subject: 'refreshToken',
            expiresIn: config.refreshTokenExpiresIn
        })

        await userRefreshTokens.insert({ refreshToken: newRefreshToken, userId: decoded.userId })
